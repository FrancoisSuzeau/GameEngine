...
namespace Component
{
	...
	public:
		TexturedComponent(glm::vec3 const position, glm::vec3 const size, Enums::RendererType const type, std::string const texture_name,
			...
			Enums::LightType const light_type = Enums::LightType::NORMALIGHT, glm::vec3 direction = glm::vec3(0.f), float const cut_off = 12.5f, float const outer_cut_off = 5.f, bool const is_attenuation = false,
			float const intensity = 0.f, glm::vec3 const angles = glm::vec3(0.f), std::string const model_type = std::string());
		...
		void SetModelType(std::string const new_val) override;
